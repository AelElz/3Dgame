...
/*   Updated: 2025/10/27 16:40:54 by ayoub            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_init(t_ray *r, const t_player *p, int screen_x)
{
	...
	/* camX in [-1,1] across the screen width */
	camX = 2.0 * (double)screen_x / (double)(WIN_W - 1) - 1.0;

	/* ray.dir = p->dir + p->plane * camX */
	...
	/* delta distances; guard against 0 */
	if (r->dir.x == 0.0)
		r->delta_dist.x = 1e30;
	else
...
void	ray_dda(t_ray *r, const t_map *m)
{
	...
	while (r->hit == 0 && safety < 100000)
	{
		...
		/* treat space as void -> solid border */
...
{
	/* side==0 means we stepped in x (hit a vertical gridline) */
	if (r->side == 0)
	{
		if (r->dir.x > 0.0)
			return (&tx->we); /* looking +x, wall face is west-facing */
		return (&tx->ea);     /* looking -x, wall face is east-facing */
	}
	/* side==1 means we stepped in y (hit a horizontal gridline) */
	if (r->dir.y > 0.0)
		return (&tx->no);     /* looking +y, wall face is north-facing */
	return (&tx->so);         /* looking -y, wall face is south-facing */
}